...
class BrechaOfertaDemandaDialog extends JDialog {

    BrechaOfertaDemandaDialog(Window owner, List<BrechaOfertaDemandaDTO> resultados) {
        super(owner, "Brecha oferta vs. demanda por área laboral",
                Dialog.ModalityType.APPLICATION_MODAL);
        setIconImage(UIUtils.image("icono.png"));

        List<BrechaOfertaDemandaDTO> datos = resultados == null
        ...
        JScrollPane graficoScroll = UIUtils.scrollable(new BrechaOfertaDemandaGrafico(datos));
    ...
    private JPanel crearPestanaTabla(List<BrechaOfertaDemandaDTO> resultados) {
        ...
        JTable tabla = new JTable(new BrechaOfertaDemandaTableModel(resultados));
        UIUtils.configureTable(tabla);
        tabla.setAutoCreateRowSorter(true);
        alinearNumeros(tabla);
        tabla.getColumnModel().getColumn(0).setPreferredWidth(UIUtils.scale(190));
    ...
    private JPanel crearIndicadores(List<BrechaOfertaDemandaDTO> resultados) {
        int ofertas = 0;
        int candidatos = 0;
        int areasConEscasez = 0;
        for (BrechaOfertaDemandaDTO resultado : resultados) {
            ofertas += resultado.getOfertasActivas();
            candidatos += resultado.getCandidatosDesempleados();
            if (resultado.getBalance() < 0) {
                areasConEscasez++;
            }
        }
        ...
        indicadores.add(crearIndicador("Ofertas activas", ofertas));
        indicadores.add(crearIndicador("Candidatos desempleados", candidatos));
        indicadores.add(crearIndicador("Áreas con escasez", areasConEscasez));
        return indicadores;
    }
